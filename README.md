...
|`ksyms`       | Linux-style kernel symbol table (`address type name` per line; modern Linux calls it `kallsyms`). macOS doesn't expose the running kernel's symbols at runtime (KASLR/SIP), so the `procfsd` daemon reads them from the running kernel's on-disk image — the arch/SoC-specific Mach-O under `/System/Library/Kernels` (e.g. `kernel.release.t8103` on an M1, selected from `kern.version`) — and returns `nm`-style lines with the static (unslid) link addresses. Mirroring Linux's `kptr_restrict`, root sees the real addresses and every other reader gets the address column zeroed. Empty without a connected daemon |
